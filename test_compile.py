                            ...
                            for ks in [7, 5, 3]:
                                for stride in [2, 1]:
                                    ...
                                    try:
                                        ...
                                        c = Conv(1, 1, ks, perf_stride=(p1, p2), padding=pad, bias=bias, stride=stride).cuda()
                                        c.eval()
                                        c2 = torch.nn.Conv2d(1, 1, ks, padding=pad, bias=bias, stride=stride).cuda()
                                        with torch.no_grad():
                                        ...
                                        t1 = c(torch.arange(0, 1 * inx * iny, 1, dtype=torch.float32).reshape(1, 1, inx, iny).cuda())
                                        l = t1.sum()
                                        l.backward()
                                        t2 = c2(torch.arange(0, 1 * inx * iny, 1, dtype=torch.float32).reshape(1, 1, inx, iny).cuda())
                                        diff = t1 - t2
                                        continue
