...
for confs in [ [torch.randn((10,10)) * 100,torch.randn((10,10)) * 100,torch.randn((10,10)) * 100, torch.randn((10,10)) * 100]]:
    if len(confs) == 1:
        perf_type = None
    else:
        perf_type = False
    ...
    n_samp = 10000
    fig, ax = plt.subplots(len(confs), 1, figsize=(5, 20) if len(confs) != 1 else (6,5))
    if len(confs) == 1:
        ax = [ax]
